...
import { requireEmpresa, ROLES_AUTORIZAR_COMPRA, ROLES_AUTORIZAR_COMPRA_MECANICO } from "@/lib/permisos";
import { Badge } from "@/components/ui/badge";
import { AutorizacionAcciones } from "./autorizacion-acciones";
import { AutorizacionAccionesMantenimiento } from "./autorizacion-acciones-mantenimiento";
...
export default async function AutorizacionesPage({
  ...
  searchParams: Promise<{ estado?: string; estadoMant?: string }>;
}) {
  const { estado, estadoMant } = await searchParams;
  const { user, prisma } = await requireEmpresa([
    ...new Set([...ROLES_AUTORIZAR_COMPRA, ...ROLES_AUTORIZAR_COMPRA_MECANICO]),
  ]);

  const puedeGerencia = ROLES_AUTORIZAR_COMPRA.includes(user.rol);
  const puedeMantenimiento = ROLES_AUTORIZAR_COMPRA_MECANICO.includes(user.rol);

  const filtro = (estado as EstadoAutorizacionCompra | undefined) ?? "PENDIENTE";
  const filtroMant = (estadoMant as EstadoAutorizacionCompra | undefined) ?? "PENDIENTE";

  const [compras, comprasMant] = await Promise.all([
    puedeGerencia
    ...
    puedeMantenimiento
      ? prisma.ordenCompra.findMany({
          where: { eliminadoEn: null, estadoAutorizacionMantenimiento: filtroMant },
          include: {
            items: { include: { archivo: true } },
            creadoPor: true,
            ordenDeTrabajo: true,
            autorizadoMantenimientoPor: true,
          },
          orderBy: { createdAt: "desc" },
        })
      : Promise.resolve([]),
    ...
    <div className="space-y-10">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Órdenes de compra que superan alguno de los montos configurados en Mantenedor/Parámetros
          y necesitan tu aprobación antes de poder comprarse.
        </p>
      </div>

      {puedeGerencia && (
        <div className="space-y-4">
          <h2 className="text-lg font-semibold">Autorización de gerencia</h2>
      ...
      {puedeMantenimiento && (
        <div className="space-y-4">
          <h2 className="text-lg font-semibold">Autorización de mantenimiento (compras de mecánicos)</h2>
          <div className="flex flex-wrap gap-2">
            {PESTANAS.map((p) => (
              <Link key={p} href={`/autorizaciones?estadoMant=${p}`}>
                <Badge variant={filtroMant === p ? ESTADO_VARIANT[p] : "outline"}>{ESTADO_LABEL[p]}</Badge>
              </Link>
            ))}
          </div>
              ...
              <div key={c.id} className="space-y-3 rounded-lg border p-4 text-sm">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="font-medium">{c.numero}</span>
                      <Badge variant={ESTADO_VARIANT[c.estadoAutorizacionMantenimiento]}>
                        {ESTADO_LABEL[c.estadoAutorizacionMantenimiento]}
                      </Badge>
                    </div>
                    <p className="text-xs text-muted-foreground">
                      Pedida el {c.fechaSolicitud.toLocaleDateString("es-AR")}
                ...
                {c.estadoAutorizacionMantenimiento === "PENDIENTE" ? (
                  <AutorizacionAccionesMantenimiento compraId={c.id} />
                ) : (
                  c.autorizadoMantenimientoPor && (
                    <p className="text-xs text-muted-foreground">
                      {ESTADO_LABEL[c.estadoAutorizacionMantenimiento]} por {c.autorizadoMantenimientoPor.nombre}
                      {c.autorizadoMantenimientoEn
                        ? ` el ${c.autorizadoMantenimientoEn.toLocaleDateString("es-AR")}`
                        : ""}
                    </p>
                  )
                )}
